...
import {CompletedProjectCard} from './ProjectCard';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { faChevronRight,  faChevronLeft} from "@fortawesome/free-solid-svg-icons";

const CompleteProjectTab = ({activeTab}) => {
  ...
  console.log('array_of_pages', array_of_pages);
      ...
      <div
        ...
        >
          ...
          {current_data.map((item, index) =>
            <CompletedProjectCard key={index} id={item.id} title={item.title} description={item.body}/>
          )}
      </div>
       ...
       <div className="w-[90%] mx-auto flex flex-col mini:flex-row justify-between items-center my-4" aria-label="Pagination navigation">
        ...
        {/* 1/3 */}
        <p className="w-12 h-12 rounded-full bg-[#001233] text-white flex flex-row items-center justify-center font-bold text-[12px]"
          role="status" aria-label={`Page ${current_page} of ${total_page_no}`}>
          <span className="text-[#FF595A]">{current_page}&nbsp;</span>/&nbsp;{total_page_no}
        </p>

        <div className="flex flex-row rounded-full bg-[#001233] px-2 border-[#001233] border-2">
...
export default CompleteProjectTab